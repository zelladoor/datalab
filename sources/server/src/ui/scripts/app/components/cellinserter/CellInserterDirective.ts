...
class CellInserterController {
  ...
  constructor (rootScope: ng.IRootScopeService, scope: CellInserterScope) {
  ...
  insertCodeCell () {
    this._emitAddCellAction(cells.code);
  }

  insertHeadingCell () {
    this._emitAddCellAction(cells.heading);
  }

  insertMarkdownCell () {
    this._emitAddCellAction(cells.markdown);
  }

  _emitAddCellAction (cellType: string) {
    var addCellAction: app.notebook.action.AddCell = {
      ...
      type: cellType,
      source: '',
      insertAfter: this._scope.insertAfterCellId
    };
    this._rootScope.$emit(addCellAction.action, addCellAction);
  }
