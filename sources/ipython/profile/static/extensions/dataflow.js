...
define([ 'd3', 'dagred3', 'visualization!table' ], function(d3, dagre, visualization) {
  ...
  function createDataTable(data) {
    ...
    var firstItemType = typeof firstItem;
    if (firstItemType == "object") {
      var transforms = [];

      Object.keys(firstItem).forEach(function(n) {
        var transform = false;

        var type = typeof firstItem[n];
        if (type == 'object') {
          type = 'string';
          transform = true;
        }

        cols.push({ id: n, label: n, type: type })
      ...
      cols.push({ id: 'item', label: 'item', type: firstItemType });
      rows = data.map(function(value) {
        return { c: [{ v: value }] };
      });
    }

    return new visualization.DataTable({ cols: cols, rows: rows });
  }
  ...
  function Pipeline(graph, data) {
    this._graph = extendOutgoingEdges(graph);
    this._data = data;
  }
  ...
  Pipeline.prototype.getNodeData = function(id) {
    var query = { id: id };
    ...
    if (node) {
      if (node.resultId) {
        return this.getNodeData(node.resultId);
      }
  ...
  PipelineGraph.prototype._onNodeClick = function(id) {
    ...
    var collection = this._pipeline.getNodeData(id);
    if (collection) {
      var dataTable = createDataTable(collection.data);
      var rowLimit = 25;
      var pageInfo = '';

      if (dataTable.getNumberOfRows() < collection.count) {
        pageInfo = ' (items 1...' + dataTable.getNumberOfRows() + ' of ' + collection.count + ')';
      }
  ...
  function renderPipeline(dom, graph, data) {
    ...
    var pipeline = new Pipeline(graph, data);
